  ...
  const getFilteredCount = (): number => {
    ...
    if (formData.dateFrom && formData.dateTo) {
      filtered = filtered.filter(t => 
        t.date >= formData.dateFrom! && t.date <= formData.dateTo!
      );
    }
    ...
    if (formData.categoryIds?.length) {
      filtered = filtered.filter(t => formData.categoryIds!.includes(t.categoryId));
    }
        ...
        <form onSubmit={handleSubmit(handleExport)} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="mt-2 text-sm text-gray-600">
              {watchedFormat === 'xlsx' && 'Formato ideal para análise de dados no Excel ou Google Sheets'}
              {watchedFormat === 'csv' && 'Formato compatível com a maioria dos softwares de planilha'}
              {watchedFormat === 'json' && 'Formato estruturado para desenvolvedores e APIs'}
              {watchedFormat === 'markdown' && 'Formato legível para documentação e relatórios'}
            </div>
          </div>
          ...
          <div>
            <Input
              ...
              required
              helperText={`O arquivo será salvo como: ${watchedFilename}.${getFileExtension(watchedFormat)}`}
            />
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <label className="flex items-center space-x-3">
                ...
                />
                <div>
                  <span className="text-sm font-medium">Incluir Metadados</span>
                  <p className="text-xs text-gray-600">
                    Data de exportação, filtros aplicados e informações do sistema
                  </p>
                </div>
              </label>
              ...
              <label className="flex items-center space-x-3">
                ...
                />
                <div>
                  <span className="text-sm font-medium">Incluir Resumo</span>
                  <p className="text-xs text-gray-600">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-gray-700 mb-2">Preview da Exportação</h3>
            <div className="text-sm text-gray-600 space-y-1">
              <p>
                <span className="font-medium">Transações a exportar:</span> {getFilteredCount()} de {transactions.length}
              </p>
              <p>
                <span className="font-medium">Formato:</span> {watchedFormat.toUpperCase()}
              </p>
              <p>
                <span className="font-medium">Tipo:</span> {
                  watchedType === 'all' ? 'Todas' : 
                  watchedType === 'income' ? 'Receitas' : 'Despesas'
                }
              </p>
            </div>
          </div>
