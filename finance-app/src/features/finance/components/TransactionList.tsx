  ...
  const filteredAndSortedTransactions = useMemo(() => {
    ...
    if (localFilters.search) {
      ...
      filtered = filtered.filter(transaction => 
        ...
        categories.find(cat => cat.id === transaction.categoryId)?.name.toLowerCase().includes(searchTerm)
      );
    }

    // Apply category filter
    if (localFilters.categoryId) {
      filtered = filtered.filter(transaction => transaction.categoryId === localFilters.categoryId);
    }

    // Apply type filter
    if (localFilters.type) {
      filtered = filtered.filter(transaction => transaction.type === localFilters.type);
    }

    // Apply date range filters
    if (localFilters.dateFrom) {
      filtered = filtered.filter(transaction => transaction.date >= localFilters.dateFrom!);
    }
    
    if (localFilters.dateTo) {
      filtered = filtered.filter(transaction => transaction.date <= localFilters.dateTo!);
    }
  ...
  return (
    <div className="bg-white shadow rounded-lg overflow-hidden">
      ...
      <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
        ...
        {Object.keys(localFilters).some(key => localFilters[key as keyof TransactionFilters]) && (
          <div className="mt-3">
            <Button
              variant="secondary"
              size="sm"
